...
docker run --rm --platform linux/amd64  --volume $(pwd):/build pi4j/pi4j-builder-repo:latest .build/build-debs.sh

# build HOMESEER.SH [V4] distribution APT repository metadata
docker run --rm --platform linux/amd64 --volume $(pwd):/build pi4j/pi4j-builder-repo:latest .build/build-repo-v4.sh
